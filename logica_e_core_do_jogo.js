// --- 6. CONTROLO DO JOGO E CÁLCULOS TRI ---
window.saveProgress = function() { 
    ...
    window.writeJSONKey(window.STORAGE_KEYS.state, { 
        ...
        missionId: window.CURRENT_MISSION_ID || null // Blindagem para o Cofre de Recuperação
    }); 
};
...
window.resumeGame = function() { 
    ...
    if(!st || !Array.isArray(st.teams) || !Array.isArray(st.activeQuestions) || !st.activeQuestions.length) { 
        ...
        window.showSystemMessage("Aviso", "Não há partida salva válida para retomar.", "info"); 
        return; 
    } 
    ...
    window.el('hud-team').style.display = window.gameMode === 'multi' ? 'flex' : 'none'; 
...
window.recordAnswerSnapshot = function(question, selectedIndex, wasCorrect, team, reason = 'answer') { 
    window.answerHistory.push({ 
        ...
        teamName: team ? team.name : null, selectedIndex, selectedText: (question && selectedIndex !== null && selectedIndex >= 0) ? question.options[selectedIndex] : null, 
        correctIndex: question ? question.answer : null, wasCorrect, reason, levelAfter: team ? team.level : null, timestamp: new Date().toISOString() 
    }); 
    window.saveProgress(); 
};
        ...
        if(isAuto) {
            ...
            setTimeout(() => { 
                ...
                setTimeout(() => { 
                    ...
                    transScreen.classList.remove('flex'); 
...
window.loadQuestion = function() {
    ...
    void qWrapper.offsetWidth; 
    qWrapper.classList.add('animate-q-slide');
    ...
    const qTextEl = window.el('q-text'); 
    qTextEl.innerHTML = ''; 
    qTextEl.classList.add('typing-cursor');
    let txt = q.text; 
    let idx = 0; 
    if(window.typeWriterTimeout) clearTimeout(window.typeWriterTimeout);
    function type() { 
        if(idx < txt.length) { 
            qTextEl.innerHTML = txt.substring(0, idx+1); 
            idx++; 
            window.typeWriterTimeout = setTimeout(type, 25); 
        } else { 
            qTextEl.classList.remove('typing-cursor'); 
        } 
    }
    type();
    
    const btnHologram = window.el('btn-toggle-hologram'); 
    const dragImgEl = window.el('drag-q-image'); 
    window.closeDraggableHologram(); 
    btnHologram.classList.add('hidden'); 
    btnHologram.classList.remove('flex'); 
    dragImgEl.src = "";
    
    if (q.image_url) { 
        const finalUrl = window.normalizeImageUrl(q.image_url); 
        dragImgEl.onload = () => { btnHologram.classList.remove('hidden'); btnHologram.classList.add('flex'); }; 
    ...
    const container = window.el('alternatives-container'); 
    container.innerHTML = ''; 
    const letters = ['A', 'B', 'C', 'D'];
    window.audioSystem.play('pergunta'); 
    if (team.level === 0) { 
        setTimeout(() => { if (team.level === 0 && !window.isWaitingAnswer) window.audioSystem.play('voice_pergunta'); }, 3500); 
    } else window.audioSystem.play('voice_proxima');
...
window.resetTimer = function() { 
    ...
    window.timeLeft = 30; 
    const timerCircle = window.el('timer-circle'); 
    const timerDisplay = window.el('timer-display'); 
    timerCircle.classList.remove('timer-panic'); 
    timerDisplay.classList.remove('text-red-500'); 
    window.el('game-vignette').classList.remove('vignette-panic'); 
    timerDisplay.innerText = window.timeLeft; 
...
window.forceTimeOut = function() { 
    ...
    const team = window.teams[window.currentTeamIndex]; 
    team.status = 'lost'; 
    window.teams[window.currentTeamIndex].responseTimes.push(30000); 
    window.audioSystem.stopAll(); window.audioSystem.play('errou'); 
    window.isWaitingAnswer = true; 
    window.el('spot-1').className = 'spotlight spot-left spot-red'; 
    window.el('spot-2').className = 'spotlight spot-right spot-red'; 
    window.el('main-q-box').classList.add('animate-shake'); 
    window.changeBrutusPose('erro'); 
    
    const q = window.activeQuestions[window.globalQuestionIndex]; 
    window.lastAnsweredQuestion = q; 
    window.recordAnswerSnapshot(q, null, false, team, 'timeout'); 
    
    window.qsa('.btn-alternative').forEach((btn, idx) => { 
        btn.style.pointerEvents = 'none'; 
        if(idx !== q.answer) btn.classList.add('fade-out-wrong-btn'); 
        else btn.classList.add('correct'); 
    }); 
    
    const tempoAudio = Math.random() > 0.5 ? 'voice_tempo1' : 'voice_tempo2'; 
    window.audioSystem.play(tempoAudio); 
    let tempoTriggered = false; 
    const goTempo = () => { 
        if(tempoTriggered) return; 
        tempoTriggered = true; 
        setTimeout(() => { 
            window.el('main-q-box').classList.remove('animate-shake'); 
            window.showFeedbackAndNext("Tempo Esgotado!", 'time'); 
        }, 800); 
    }; 
    window.audioSystem[tempoAudio].onended = goTempo; 
    window.audioSystem[tempoAudio].onerror = goTempo; 
...
window.selectAnswer = function(selectedIndex, buttonElement) { 
    ...
    clearInterval(window.timerInterval); 
    window.el('timer-circle').classList.remove('timer-panic'); 
    window.el('game-vignette').classList.remove('vignette-panic'); 
    window.pendingAnswerIndex = selectedIndex; 
    window.pendingButtonElement = buttonElement; 
    if (window.questionStartTime) { window.teams[window.currentTeamIndex].responseTimes.push(Date.now() - window.questionStartTime); } 
    
    window.changeBrutusPose('tenso_cinematic'); 
    window.el('spot-1').className = 'spotlight spot-left spot-tension'; 
    window.el('spot-2').className = 'spotlight spot-right spot-tension'; 
    
    const q = window.activeQuestions[window.globalQuestionIndex]; 
    const letters = ['A', 'B', 'C', 'D']; 
    window.el('confirm-letter').innerText = letters[selectedIndex]; 
    window.el('confirm-text').innerText = q.options[selectedIndex]; 
    buttonElement.classList.add('pulse-answer'); 
    window.el('modal-confirm').classList.remove('hidden'); 
    window.el('modal-confirm').classList.add('flex'); 
    window.audioSystem.stop('suspense'); window.audioSystem.play('certeza'); 
    const voiceName = Math.random() > 0.5 ? 'voice_certeza' : 'voice_posso'; 
    window.audioSystem.play(voiceName); 
    const btnSim = window.el('btn-confirm-sim'); const btnNao = window.el('btn-confirm-nao'); 
    btnSim.classList.add('opacity-40', 'pointer-events-none', 'grayscale'); 
    btnNao.classList.add('opacity-40', 'pointer-events-none', 'grayscale'); 
    const enableButtons = () => { 
        btnSim.classList.remove('opacity-40', 'pointer-events-none', 'grayscale'); 
        btnNao.classList.remove('opacity-40', 'pointer-events-none', 'grayscale'); 
    }; 
    window.audioSystem[voiceName].onended = enableButtons; 
    window.audioSystem[voiceName].onerror = enableButtons; 
    setTimeout(enableButtons, 3500); 
...
window.cancelAnswer = function() { 
    window.teams[window.currentTeamIndex].responseTimes.pop(); 
    window.questionStartTime = Date.now(); 
    window.changeBrutusPose('normal'); 
    window.el('spot-1').className = 'spotlight spot-left spot-white'; 
    window.el('spot-2').className = 'spotlight spot-right spot-white'; 
    window.el('confirm-box').classList.remove('scale-100'); 
    window.el('confirm-box').classList.add('scale-95'); 
    ...
    window.audioSystem.stop('certeza'); window.audioSystem.play('suspense', true); 
    window.resumeTimer(); 
};

window.confirmAnswer = function() { 
    window.el('modal-confirm').classList.add('hidden'); 
    window.el('modal-confirm').classList.remove('flex'); 
    window.isWaitingAnswer = true; 
    const team = window.teams[window.currentTeamIndex]; 
    const q = window.activeQuestions[window.globalQuestionIndex]; 
    
    window.el('bg-game').classList.add('dim-bg-extreme'); 
    window.el('char-host').classList.add('dim-bg-extreme'); 
    window.el('question-panel-wrapper').classList.add('dim-bg-extreme'); 
    window.el('spot-1').className = 'spotlight spot-left'; 
    window.el('spot-2').className = 'spotlight spot-right'; 
    window.el('spot-1').style.opacity = '0'; 
    window.el('spot-2').style.opacity = '0'; 
    
    window.qsa('.btn-alternative').forEach(btn => { 
        if(btn !== window.pendingButtonElement) { btn.classList.add('tension-dim'); btn.style.pointerEvents = 'none'; } 
    }); 
    window.pendingButtonElement.classList.remove('pulse-answer'); 
    window.pendingButtonElement.classList.add('animate-suspense', 'tension-focus'); 
    
    window.tensionFlashesInterval = setInterval(() => { 
        if(Math.random() > 0.5) { 
            const f = window.el('camera-flash-overlay'); 
            f.classList.remove('do-flash'); void f.offsetWidth; f.classList.add('do-flash'); 
        } 
    }, 600); 
    
    setTimeout(() => { 
        clearInterval(window.tensionFlashesInterval); 
        window.pendingButtonElement.classList.remove('animate-suspense', 'tension-focus'); 
        window.el('bg-game').classList.remove('dim-bg-extreme'); 
        window.el('char-host').classList.remove('dim-bg-extreme'); 
        window.el('question-panel-wrapper').classList.remove('dim-bg-extreme'); 
        window.qsa('.btn-alternative').forEach(btn => btn.classList.remove('tension-dim')); 
        window.el('spot-1').style.opacity = ''; window.el('spot-2').style.opacity = ''; 
        
        const isCorrect = window.pendingAnswerIndex === q.answer; 
        if (isCorrect) { 
            window.changeBrutusPose('acerto'); 
            window.el('spot-1').className = 'spotlight spot-left spot-green'; 
            window.el('spot-2').className = 'spotlight spot-right spot-green'; 
            team.level++; 
            window.globalQuestionIndex++; 
            window.lastAnsweredQuestion = q; 
            window.recordAnswerSnapshot(q, window.pendingAnswerIndex, true, team); 
            window.audioSystem.stopAll(); window.audioSystem.play('certa'); window.audioSystem.play('voice_acerto'); 
            window.pendingButtonElement.classList.add('flash-correct'); 
            window.el('award-display').classList.add('animate-award-pop'); 
            window.qsa('.btn-alternative').forEach((btn, idx) => { if(idx !== q.answer) btn.classList.add('fade-out-wrong-btn'); }); 
            
            const floatScore = window.ce('div'); 
            floatScore.className = 'floating-score'; 
            floatScore.innerText = `+${window.awards[team.level-1]}`; 
            window.pendingButtonElement.appendChild(floatScore); 
            
            let nextActionTriggered = false; 
            const goNext = () => { 
                if (nextActionTriggered) return; 
                nextActionTriggered = true; 
                setTimeout(() => { 
                    const gameScreen = window.el('screen-game'); 
                    gameScreen.style.transition = 'opacity 0.6s ease'; 
                    gameScreen.style.opacity = '0'; 
                    setTimeout(() => { 
                        if(team.level < 16) { 
                            gameScreen.style.opacity = '1'; 
                            window.advanceToNextTurn(); 
                        } else { 
                            team.status = 'won'; 
                            window.audioSystem.stopAll(); window.audioSystem.play('vitoria'); 
                            window.el('final-win-award').innerText = "1 MILHÃO"; 
                            window.el('end-win-team').innerText = window.isStudentMode ? `Fim de Treino: ${team.name}` : `Equipe Campeã: ${team.name}`; 
                            const winScreen = window.el('screen-end-win'); 
                            winScreen.classList.remove('hidden'); winScreen.classList.add('active'); 
                            window.checkGameEnd('win'); 
                            setTimeout(() => { 
                                const winBox = window.el('win-box'); 
                                winBox.classList.remove('scale-90', 'opacity-0'); 
            ...
            window.audioSystem.voice_acerto.onended = goNext; 
            window.audioSystem.voice_acerto.onerror = goNext; 
            ...
            window.globalQuestionIndex++; 
            window.lastAnsweredQuestion = q; 
            window.recordAnswerSnapshot(q, window.pendingAnswerIndex, false, team); 
            window.audioSystem.stopAll(); window.audioSystem.play('errou'); window.audioSystem.play('voice_errou'); 
            window.pendingButtonElement.classList.add('wrong'); 
            window.qsa('.btn-alternative')[q.answer].classList.add('correct'); 
            window.qsa('.btn-alternative').forEach((btn, idx) => { if(idx !== q.answer) btn.classList.add('fade-out-wrong-btn'); }); 
            
            let erroTriggered = false; 
            const goErro = () => { 
                if (erroTriggered) return; 
                erroTriggered = true; 
                setTimeout(() => { 
                    window.el('main-q-box').classList.remove('animate-shake'); 
                    window.showFeedbackAndNext("Resposta Incorreta!", 'wrong'); 
                }, 800); 
            }; 
            window.audioSystem.voice_errou.onended = goErro; 
            window.audioSystem.voice_errou.onerror = goErro; 
            setTimeout(goErro, 8000); 
        } 
    }, 5000); 
};
...
window.checkGameEnd = function(screenType) { 
    ...
    const btnText = window.el(`btn-end-${screenType}-text`); 
    if (activeCount === 0) { 
        btnText.innerText = "VER RESULTADOS"; 
        btn.onclick = () => { window.el(`screen-end-${screenType}`).classList.remove('active'); window.showLeaderboard(); }; 
    } else { 
        btnText.innerText = "CONTINUAR JOGO"; 
        btn.onclick = () => { 
            window.el(`screen-end-${screenType}`).style.opacity = '0'; 
            setTimeout(() => { 
                window.el(`screen-end-${screenType}`).classList.remove('active'); 
                window.el('screen-game').classList.add('active'); 
                window.advanceToNextTurn(); 
            }, 500); 
        }; 
...
window.advanceToNextTurn = function() { 
    let next = (window.currentTeamIndex + 1) % window.teams.length; 
    let found = false; 
    let loops = 0; 
    while (loops < window.teams.length) { 
        if (window.teams[next].status === 'playing') { found = true; break; } 
        next = (next + 1) % window.teams.length; loops++; 
    } 
    if (found) { 
        window.currentTeamIndex = next; 
        window.showTurnTransition(window.teams[window.currentTeamIndex].name, () => { 
            window.loadQuestion(); window.audioSystem.play('suspense', true); 
        }); 
    } else { 
...
window.useHelp = function(type) { 
    const team = window.teams[window.currentTeamIndex]; 
    if (type !== 'pular' && team.helps[type]) return; 
    ...
    window.closeHelp(); 
    if (type === 'eliminar') { 
        window.audioSystem.play('eliminar'); window.pauseTimer(); team.helps.eliminar = true; window.saveProgress(); 
        const cartas = [0, 1, 2, 3].sort(() => Math.random() - 0.5); 
        const container = window.el('cards-container'); container.innerHTML = ''; 
        cartas.forEach((val) => { 
            container.innerHTML += `<div class="w-20 h-32 cursor-pointer flip-card" onclick="window.chooseCarta(${val}, this)"><div class="flip-card-inner w-full h-full relative"><div class="flip-card-front absolute inset-0 bg-blue-900 border-2 border-yellow-400 rounded-xl flex items-center justify-center text-white font-black">?</div><div class="flip-card-back absolute inset-0 bg-white border-2 border-gray-300 rounded-xl flex flex-col items-center justify-center p-2 text-black"><span class="text-blue-950 font-black font-orbitron text-xs">${val===0?'REI':val===1?'ÁS':val===2?'DUAS':'TRÊS'}</span></div></div></div>`; 
        }); 
        window.el('modal-cartas').classList.remove('hidden'); window.el('modal-cartas').classList.add('flex'); 
    } else if (type === 'palpite') { 
        window.audioSystem.play('plateia'); window.audioSystem.play('voice_palpite'); window.pauseTimer(); team.helps.palpite = true; window.saveProgress(); 
        const q = window.activeQuestions[window.globalQuestionIndex]; 
        const container = window.el('audience-bars'); container.innerHTML = ''; 
        [0, 1, 2, 3].forEach(i => { 
            let v = i === q.answer ? 65 : 11; 
            container.innerHTML += `<div class="flex items-center w-full gap-4 text-xs font-bold"><div class="w-6 h-6 rounded-full bg-blue-950 border border-cyan-400 flex items-center justify-center">${['A','B','C','D'][i]}</div><div class="w-full bg-black/50 h-6 rounded-full relative overflow-hidden"><div class="h-full bg-yellow-500" style="width:${v}%"></div><span class="absolute right-4 text-white">${v}%</span></div></div>`; 
        }); 
        window.el('modal-audience').classList.remove('hidden'); window.el('modal-audience').classList.add('flex'); 
    } else if (type === 'dica') { 
        window.audioSystem.play('dica'); window.audioSystem.play('voice_dica'); window.pauseTimer(); team.helps.dica = true; window.saveProgress(); 
        const q = window.activeQuestions[window.globalQuestionIndex]; 
        const container = window.el('dica-boards'); container.innerHTML = ''; 
        [1,2,3].forEach(idx => { 
            container.innerHTML += `<div class="flex-1 bg-blue-950 border border-yellow-500 rounded-xl p-4 text-center"><span class="text-cyan-400 text-[10px] block mb-2">Especialista ${idx}</span><div class="w-10 h-10 rounded-full bg-white flex items-center justify-center mx-auto text-red-700 font-bold mb-2">${['A','B','C','D'][q.answer]}</div><p class="text-white text-[11px] truncate">${q.options[q.answer]}</p></div>`; 
        }); 
        window.el('modal-dica').classList.remove('hidden'); window.el('modal-dica').classList.add('flex'); 
    } else if (type === 'pular') { 
        window.el('skips-left').innerText = 3 - team.helps.pular; window.el('modal-pular').classList.remove('hidden'); window.el('modal-pular').classList.add('flex'); 
    } 
};

window.chooseCarta = function(val, element) { 
    element.classList.add('flipped'); 
    window.qsa('#cards-container .flip-card').forEach(c=>c.style.pointerEvents='none'); 
    setTimeout(() => { 
        window.el('modal-cartas').classList.add('hidden'); window.el('modal-cartas').classList.remove('flex'); 
        const btns = window.qsa('#alternatives-container .btn-alternative'); 
        const q = window.activeQuestions[window.globalQuestionIndex]; 
        let wrong = [0,1,2,3].filter(i => i!==q.answer).sort(() => Math.random() - 0.5); 
        for(let k=0; k<val; k++) if(wrong[k]!==undefined) btns[wrong[k]].classList.add('animate-eliminate'); 
        window.resumeTimer(); 
    }, 2000); 
};
...
window.confirmStop = function() { 
    window.isWaitingAnswer = true; 
    window.el('modal-parar').classList.add('hidden'); window.el('modal-parar').classList.remove('flex'); 
    const val = window.teams[window.currentTeamIndex].level === 0 ? "0" : window.awards[window.teams[window.currentTeamIndex].level - 1]; 
    window.teams[window.currentTeamIndex].status = 'stopped'; 
    window.globalQuestionIndex++; window.saveProgress(); 
    window.el('final-stop-award').innerText = val; 
    window.el('end-stop-team').innerText = `Equipe Parou: ${window.teams[window.currentTeamIndex].name}`; 
    window.changeBrutusPose('consolando'); 
    window.el('screen-game').classList.remove('active'); window.el('screen-end-stop').classList.add('active'); 
    window.checkGameEnd('stop'); 
};

window.confirmSkip = function() { 
    window.cancelSkip(); window.teams[window.currentTeamIndex].helps.pular++; window.globalQuestionIndex++; window.saveProgress(); 
    if(window.globalQuestionIndex < window.activeQuestions.length) window.loadQuestion(); 
    else window.showSystemMessage("Fim", "Sem perguntas para pular.", "error"); 
};

// --- GESTÃO DE TURMAS (Com Edição de Ano Escolar) ---
window.saveClass = function() { 
    const name = window.el('add-class-name').value.trim(); 
    const year = window.el('add-class-year') ? window.el('add-class-year').value : ''; 
    if (!name) { window.el('add-class-error').classList.remove('hidden'); return; } 
    
    // Se for edição, atualiza a turma existente
    if (window.activeTurmaId && window.el('modal-add-class').dataset.mode === 'edit') {
        const idx = window.allTurmas.findIndex(t => t.id === window.activeTurmaId);
        if (idx !== -1) {
            window.allTurmas[idx].name = name;
            window.allTurmas[idx].ano = year;
        }
    } else {
        // Se for nova, adiciona
        const newClass = { id: `TURMA_${Date.now()}`, name: name, ano: year, students: [] }; 
        window.allTurmas.push(newClass); 
        window.activeTurmaId = newClass.id;
    }

    window.writeJSONKey(window.STORAGE_KEYS.classes, window.allTurmas); 
    window.closeAddClassModal(); 
    window.showSystemMessage("Sucesso", "Turma guardada com sucesso.", "success"); 
    window.renderClassList(); 
    window.updateClassDetailsView(); 
};

window.deleteCurrentClass = function() { 
    if(!window.activeTurmaId || !confirm("Excluir permanentemente esta turma e todos os seus alunos?")) return; 
    window.allTurmas = window.allTurmas.filter(t => t.id !== window.activeTurmaId); 
    window.writeJSONKey(window.STORAGE_KEYS.classes, window.allTurmas); 
...
window.saveStudent = function() { 
    const name = window.el('add-student-name').value.trim(); 
    if (!name) { window.el('add-student-error').classList.remove('hidden'); return; } 
    const idx = window.allTurmas.findIndex(t => t.id === window.activeTurmaId); 
    if(idx === -1) return; 
    window.allTurmas[idx].students.push({ 
        id: `ALUNO_${Date.now()}`, 
        name: name, 
        race: window.el('add-student-race').value, 
        isBolsa: window.el('add-student-bolsa').checked, 
        isAEE: window.el('add-student-aee').checked 
    }); 
    window.writeJSONKey(window.STORAGE_KEYS.classes, window.allTurmas); 
...
window.deleteStudent = function(sid) { 
    const idx = window.allTurmas.findIndex(t => t.id === window.activeTurmaId); 
    if(idx === -1 || !confirm("Remover aluno permanentemente?")) return; 
    window.allTurmas[idx].students = window.allTurmas[idx].students.filter(s => s.id !== sid); 
    window.writeJSONKey(window.STORAGE_KEYS.classes, window.allTurmas); 
    window.updateClassDetailsView(); 
    window.renderClassList(); 
};